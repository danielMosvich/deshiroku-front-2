...
function Collection({ user, collection }) {
  ...
  async function handleEditCollection() {
    ...
    const id = $user.collections.find(
      (e) => e.name.trim() === collection.trim()
    )._id;
    setShowEditButton(false);
    editCollection(id, sanitizedCollectionName).then((e) => {
      if (e.success) {
        if (STORE_defaultCollection.value.name === collection) {
          const resultCollection = e.data.collections.find(
            (e) => e.name === sanitizedCollectionName
          );
          localStorage.setItem(
            "defaultCollection",
            JSON.stringify(resultCollection)
          );
          STORE_defaultCollection.set(JSON.stringify(resultCollection));
        }
    ...
    <div className="relative lg:px-20 sm:px-10  px-2">
      {data ? (
        <div className="flex items-center justify-center flex-col mt-10 mb-5">
          <h2 className="text-3xl font-semibold font-ui dark:text-neutral-50 text-center">
            {data.name}
          </h2>
      ...
      {showEdit && (
        <ModalQuestion onClose={() => setShowEdit(false)}>
          <div className="bg-neutral-100/90 backdrop-blur-2xl p-4 rounded-xl md:min-w-[400px] w-full mx-auto dark:bg-neutral-900">
            <h3 className=" font-semibold text-md dark:text-white">New name collection</h3>
            <input
              className="bg-white px-3 py-3 outline-offset-4 outline-2 outline-sky-500 rounded-xl border-none mt-2 w-full dark:bg-neutral-800 dark:placeholder:text-neutral-300 dark:text-white"
              type="text"
      ...
      {showDelete && (
        <ModalQuestion onClose={() => setShowDelete(false)}>
          <ModalQuestion onClose={() => setShowDelete(false)}>
            <div className="bg-neutral-100/90 backdrop-blur-2xl p-4 py-5 rounded-xl md:min-w-[400px] w-[90%] mx-auto dark:bg-neutral-900">
              <h3 className=" font-semibold text-md text-center dark:text-white">
